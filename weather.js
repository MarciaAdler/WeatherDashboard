$(document).ready(function() {
  ...
  function populateSearchHistory() {
    ...
    $.each(cities, function(index, value) {
      var newRow = $("<tr>");
      var location = $("<button>");
      location.addClass("city-list-item").attr("data-value", value);
      location.text(value);
  ...
  function searchByCity(city) {
    ...
    var queryURL =
      ...
      city +
      "&units=imperial&appid=4074b5f07760c717f95e1eec3f81ef1f";
    $.ajax({
      url: queryURL,
      method: "GET"
      ...
      var name = $("<h2>")
        .addClass("city")
        .text(`${response.name} (${moment().format("M/D/YYYY")})`);
      var icon = $("<img>")
        .addClass("icon")
        .attr(
          "src",
    ...
    function getUVIndex(lat, lon) {
      var queryURL2 =
        ...
        "&lon=" +
        lon +
        "&appid=4074b5f07760c717f95e1eec3f81ef1f";
      $.ajax({
        url: queryURL2,
        method: "GET"
      }).then(function(response) {
  ...
  function populateFiveDay(city) {
    var queryURL =
      ...
      city +
      "&units=imperial&mode=JSON&appid=4074b5f07760c717f95e1eec3f81ef1f";
    $.ajax({
      url: queryURL,
      method: "GET"
    }).then(function(response) {
      ...
      var forecastDayOne = $("<div>").addClass(
        "card-body text-white bg-info mb-3 day-one col-sm-3"
      );
      var forecastDayTwo = $("<div>").addClass(
        "card-body text-white bg-info mb-3 day-two col-sm-3"
      );
      var forecastDayThree = $("<div>").addClass(
        "card-body text-white bg-info mb-3 day-three col-sm-3"
      );
      var forecastDayFour = $("<div>").addClass(
        "card-body text-white bg-info mb-3 day-four col-sm-3"
      );
      var forecastDayFive = $("<div>").addClass(
        "card-body text-white bg-info mb-3 day-five col-sm-3"
      );
      ...
      var dateOne = $("<h5>")
        .addClass("date")
        .text(moment(response.list[5].dt_txt).format("M/D/YYYY"));
      var dateTwo = $("<h5>")
        .addClass("date")
        .text(moment(response.list[13].dt_txt).format("M/D/YYYY"));
      var dateThree = $("<h5>")
        .addClass("date")
        .text(moment(response.list[21].dt_txt).format("M/D/YYYY"));
      var dateFour = $("<h5>")
        .addClass("date")
        .text(moment(response.list[29].dt_txt).format("M/D/YYYY"));
      var dateFive = $("<h5>")
        .addClass("date")
        .text(moment(response.list[37].dt_txt).format("M/D/YYYY"));
      ...
      var tempOne = $("<p>")
        ...
        .text(`Temp: ${response.list[5].main.temp}˚F`);
      var tempTwo = $("<p>")
        .addClass("forecast-temp")
        .text(`Temp: ${response.list[13].main.temp}˚F`);
      var tempThree = $("<p>")
        .addClass("forecast-temp")
        .text(`Temp: ${response.list[21].main.temp}˚F`);
      var tempFour = $("<p>")
        .addClass("forecast-temp")
        .text(`Temp: ${response.list[29].main.temp}˚F`);
      var tempFive = $("<p>")
        .addClass("forecast-temp")
        .text(`Temp: ${response.list[37].main.temp}˚F`);
      ...
      var humidityOne = $("<p>")
        .addClass("forecast-temp")
        .text(`Humidity: ${response.list[3].main.temp}%`);
      var humidityTwo = $("<p>")
        .addClass("forecast-temp")
        .text(`Humidity: ${response.list[11].main.temp}%`);
      var humidityThree = $("<p>")
        .addClass("forecast-temp")
        .text(`Humidity: ${response.list[19].main.temp}%`);
      var humidityFour = $("<p>")
        .addClass("forecast-temp")
        .text(`Humidity: ${response.list[27].main.temp}%`);
      var humidityFive = $("<p>")
        .addClass("forecast-temp")
        .text(`Humidity: ${response.list[35].main.temp}%`);
